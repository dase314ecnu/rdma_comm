...
void RdmaServer<T>::listenThreadFun() {
  ...
  while (true) {
    ...
    int connfd = accept(sock, (struct sockaddr*)&client_address, 
                &client_addrlength);
    if (connfd < 0) {
      LOG_DEBUG("connect file descriptor is invalid");
      assert(false);
    }
...
void RdmaServer<T>::receiveThreadFun(uint32_t node_idx) {
  ...
  waitset->addFd(this->rdma_queue_pairs[node_idx]->GetChannel()->fd);
  ...
  while (true) {
    ...
    for (int i = 0; i < rc; ++i) {
      ...
      if (wc.opcode == IBV_WC_RECV_RDMA_WITH_IMM) {
        ...
        char    *buf = this->rdma_queue_pairs[node_idx]->GetLocalMemory() + 
